...
class EndScreen():
    def __init__(self, deck, shoplist, timer, floorNumber, screenCenter, lang):
        self.lang = lang
        ...
        self.maintext = text.Text(const.gameFont(80), "Main text", (0,0))
        self.subtext = text.Text(const.gameFont(30), "more text", (0,0))
        self.readyButton = button.Button(0,4,(0,0),const.scale, const.gameFont(28), const.phrase[self.lang][12])
        self.active = False
        self.pos = screenCenter

    def activate(self, timer=-1, floorNumber=-1):
        if not self.active:
...
class WinScreen(EndScreen):
    def __init__(self, deck, shoplist, timer, floorNumber, screenCenter, lang):
        super().__init__(deck, shoplist, timer, floorNumber, screenCenter, lang)
